...
router.get('/me', authMiddleware, async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    logger.error('GET /users/me failed', { error: String(error) });
...
router.put('/me', authMiddleware, async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    logger.error('PUT /users/me failed', { error: String(error) });
    res.status(500).json({ success: false, error: 'Internal server error' });
  }
});
...
router.post('/sync', async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    logger.error('POST /users/sync failed', { error: String(error) });
    res.status(500).json({ success: false, error: 'Internal server error' });
  }
});
